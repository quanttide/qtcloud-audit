...
- **审计对象**: tests/fixtures/evidence/qtcloud_data_test_fixtures.py
- **发现总数**: 5
  - 不合规: 4
  - 警告: 1
...
qtcloud_data_test_fixtures.py 的业务意图被严重淹没在技术细节中，虽然代码质量优秀（94% 通过率），但业务人员无法理解代码意图。
...
qtcloud_data_test_fixtures.py 的代码质量优秀（94%），但业务意图表达严重不清晰（24%）。
